...
public class Passenger extends BorderPane {
	...
	private void createNavbar() {
		...
		ImageView logoImg;
		try {
			Image img = new Image(getClass().getResourceAsStream("/com/project/resources/companyLogo.png"), 80, 60,
					true, true);
	...
	private void setActiveNav(int activeIdx) {
		for (int i = 0; i < navButtons.length; i++) {
			...
			if (i == activeIdx)
				navButtons[i].getStyleClass().add("topnav-btn-active");
		}
	}

	private void clearActiveNav() {
		for (Button b : navButtons)
			b.getStyleClass().remove("topnav-btn-active");
	}
	...
	private void insertAuditLog(int uid, String status) {
		try {
			DbConnectMsSql db = new DbConnectMsSql();
			PreparedStatement ps = db.conn
					.prepareStatement("INSERT INTO Audit_Log (user_id, log_status) VALUES (?, ?)");
			ps.setInt(1, uid);
